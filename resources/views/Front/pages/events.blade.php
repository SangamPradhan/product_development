...
<section class="relative pt-48 pb-32 px-margin-desktop overflow-hidden">
    <div class="max-w-4xl" data-aos="fade-up">
        ...
        <h1 class="text-display-lg font-display-lg mb-8 font-bold">Architecting the future through collaborative intelligence.</h1>
        <p class="text-body-lg text-body-lg text-on-surface-variant max-w-2xl">Join our upcoming webinars, hands-on workshops, and flagship summits. We bridge the gap between theoretical AI research and enterprise-ready automation.</p>
    </div>
...
<section class="px-margin-desktop mb-section-gap">
    @php
        ...
        $featuredDesc = $featured ? Str::limit($featured->description, 180) : 'Our flagship event for C-level executives and lead architects. Three days of intensive strategy sessions on the future of autonomous workflows.';
        ...
        <div class="md:col-span-8 group relative overflow-hidden bg-surface-container-low border border-outline-variant/30 angled-notch-lg p-10 flex flex-col justify-end min-h-[500px] hover-glow transition-all" data-aos="fade-right">
            ...
            <div class="relative z-10">
                <div class="flex items-center gap-4 mb-4">
                    <span class="text-label-sm font-label-sm bg-secondary text-white px-2 py-1 uppercase font-bold">{{ $featuredCat }}</span>
                    <span class="text-label-sm font-label-sm text-on-surface-variant uppercase font-bold">{{ $featuredDate }}</span>
                </div>
                <h2 class="text-headline-lg font-headline-lg mb-4 font-bold">{{ $featuredTitle }}</h2>
                <div class="text-body-md font-body-md text-on-surface-variant mb-8 max-w-lg leading-relaxed">{!! $featuredDesc !!}</div>
                <a href="{{ $featuredLink }}" class="notch-button inline-block bg-secondary text-white px-8 py-3.5 font-label-sm text-label-sm hover:bg-on-secondary-fixed-variant transition-all hover-glow text-center font-bold">
        ...
        <div class="md:col-span-4 bg-surface-container-low border border-outline-variant/30 angled-notch p-8 flex flex-col justify-between hover-glow transition-all" data-aos="fade-left" data-aos-delay="100">
            <div>
                <div class="flex justify-between items-start mb-12">
                    <span class="material-symbols-outlined text-secondary text-4xl">terminal</span>
                    <span class="text-label-sm font-label-sm text-secondary font-bold">LIVE NOW</span>
                </div>
                <h3 class="text-headline-md font-headline-md mb-2 font-bold">Dev-to-Production Workshop</h3>
                <p class="text-body-md font-body-md text-on-surface-variant">Deploying LLMs at scale with zero-latency requirements.</p>
            </div>
            <a href="{{ route('front.contact') }}" class="text-secondary font-bold text-label-sm font-label-sm flex items-center gap-2 mt-8 group">
...
<section class="px-margin-desktop mb-16 border-b border-outline-variant/20 pb-8 flex flex-col md:flex-row justify-between items-end gap-8" data-aos="fade-up">
    <div>
        <h2 class="text-headline-md font-headline-md mb-2 font-bold">Filter Events by Category</h2>
    ...
    <div class="flex gap-2 overflow-x-auto pb-2 md:pb-0">
        <a href="{{ route('front.events') }}" class="px-6 py-2 text-label-sm font-label-sm rounded-full transition-colors {{ empty($category) ? 'bg-secondary text-white' : 'border border-outline-variant text-on-surface-variant hover:border-secondary' }} font-bold">All Events</a>
        @foreach($categories as $cat)
            <a href="{{ route('front.events', ['category' => $cat]) }}" 
               class="px-6 py-2 text-label-sm font-label-sm rounded-full transition-colors {{ $category === $cat ? 'bg-secondary text-white' : 'border border-outline-variant text-on-surface-variant hover:border-secondary' }} font-bold uppercase">{{ $cat }}</a>
        @endforeach
    </div>
</section>
...
<section class="px-margin-desktop mb-section-gap">
    <h2 class="text-headline-lg font-headline-lg mb-8 font-bold">Upcoming Events</h2>
        ...
        @forelse($upcomingEvents as $idx => $event)
            ...
            <div onclick="window.location.href='{{ route('front.event.detail', $event->slug) }}'" 
                ...
                <div>
                    ...
                    <div class="text-secondary font-label-sm text-label-sm mb-3 flex items-center gap-2 uppercase font-bold">
                        <span class="material-symbols-outlined text-[16px]">calendar_today</span>
                        {{ $event->event_date->format('M d, Y') }}
                    </div>
                    <h4 class="text-headline-md font-headline-md text-on-surface mb-3 group-hover:text-secondary transition-colors font-bold leading-snug">{{ $event->title }}</h4>
                    <p class="text-body-md font-body-md text-on-surface-variant mb-6 line-clamp-3 leading-relaxed">{{ strip_tags($event->description) }}</p>
                </div>
                <div class="flex justify-between items-center mt-4 border-t border-outline-variant/10 pt-4">
                    <span class="text-xs font-label-sm uppercase text-secondary font-bold">{{ $event->category }}</span>
                    <button class="text-secondary font-bold text-label-sm font-label-sm underline flex items-center gap-1">
                        Register / Details <span class="material-symbols-outlined text-xs">arrow_forward</span>
                    </button>
                </div>
            </div>
        @empty
            <div class="md:col-span-3 p-12 text-center text-on-surface-variant font-label-sm uppercase bg-surface-container-low border border-outline border-dashed rounded-xl">
                No upcoming events found.
            </div>
        @endforelse

        <!-- Host a custom workshop CTA Card -->
        <div class="md:col-span-3 bg-secondary p-8 angled-notch flex flex-col md:flex-row items-center justify-between gap-8 hover-glow transition-all mt-8" data-aos="fade-up">
            <div class="text-white">
                <h4 class="text-headline-md font-headline-md mb-2 font-bold">Host a custom workshop?</h4>
            ...
            <a href="{{ route('front.contact') }}" class="notch-button whitespace-nowrap bg-white text-secondary px-8 py-3.5 font-label-sm text-label-sm hover:bg-secondary-container transition-all hover-glow text-center font-bold">
...
<section class="px-margin-desktop mb-section-gap">
    <h2 class="text-headline-lg font-headline-lg mb-8 font-bold">Past Events / Archive</h2>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-gutter">
        @forelse($pastEvents as $idx => $event)
            ...
            <div onclick="window.location.href='{{ route('front.event.detail', $event->slug) }}'" 
                ...
                <div>
                    ...
                    <div class="text-on-surface-variant/60 font-label-sm text-label-sm mb-3 flex items-center gap-2 uppercase font-bold">
                        <span class="material-symbols-outlined text-[16px]">history</span>
                        CONCLUDED {{ $event->event_date->format('M d, Y') }}
                    </div>
                    <h4 class="text-headline-md font-headline-md text-on-surface/80 mb-3 group-hover:text-secondary transition-colors font-bold leading-snug">{{ $event->title }}</h4>
                    <p class="text-body-md font-body-md text-on-surface-variant mb-6 line-clamp-3 leading-relaxed">{{ strip_tags($event->description) }}</p>
                </div>
                <div class="flex justify-between items-center mt-4 border-t border-outline-variant/10 pt-4">
                    <span class="text-xs font-label-sm uppercase text-on-surface-variant/60 font-bold">{{ $event->category }}</span>
                    <button class="text-on-surface-variant/80 font-bold text-label-sm font-label-sm underline flex items-center gap-1">
...
<section class="bg-surface-container-lowest py-32 px-margin-desktop border-t border-outline-variant/20" data-aos="fade-in">
    <div class="max-w-4xl mx-auto text-center" data-aos="zoom-in">
        <h2 class="text-headline-lg font-headline-lg mb-6 font-bold">Stay informed. No noise.</h2>
        ...
        <form class="flex flex-col md:flex-row gap-4 max-w-xl mx-auto" onsubmit="event.preventDefault(); alert('Subscribed successfully!');">
            ...
            <button class="notch-button bg-secondary text-white px-8 py-4 font-label-sm text-label-sm hover:bg-on-secondary-fixed-variant transition-all hover-glow font-bold uppercase tracking-wider" type="submit">
                Subscribe
            </button>
        </form>
    </div>
